...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start_never_used, t, model_kwargs=None, noise=None):
        ...
        # The anchor loss
        decoder_nll = self.anchor_loss(x_start_mean, get_logits, input_ids)
    ...
    def anchor_loss(self, x_t, get_logits, input_ids):
        logits = get_logits(x_t)
        loss = th.nn.CrossEntropyLoss(reduction="none")
        decoder_nll = loss(logits.view(-1, logits.size(-1)), input_ids.view(-1)).view(input_ids.shape).mean(dim=-1)
        return decoder_nll
    ...
    def _vb_terms_bpd_e2e(
        ...
        decoder_nll = self.anchor_loss(x_start, get_logits, input_ids)  # t=-1
        # print("xtshape", x_t.shape, "xtstartshape", x_start.shape)
        # decoder_nll = self.token_discrete_loss(x_t, get_logits, input_ids)  # t=-1
